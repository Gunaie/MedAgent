...
# ==================== 医疗图谱查询模板 ====================
# 全部使用 WHERE 形式，便于 fuzzy 时安全替换为 CONTAINS
GRAPH_TEMPLATE: Final[dict] = {
    "desc": {
        ...
        "cypher": """
            ...
            RETURN d.name AS name, collect(DISTINCT s.name) AS RES
            LIMIT 1
        """,
        "answer": "【{disease}】{name}是一种疾病，常见症状包括：{RES}。",
    },
    "cause": {
        "slots": ["disease"],
        "question": "{disease}一般是由什么引起的？",
        "cypher": "MATCH (n:Disease) WHERE n.name = $disease RETURN coalesce(n.infection, '具体病因需结合临床检查确定') AS RES LIMIT 1",
    ...
    "cure_way": {
        ...
        "cypher": """
            MATCH (d:Disease)-[:TREATED_WITH]->(drug:Drug)
            WHERE d.name = $disease
    ...
    "check": {
        ...
        "cypher": """
            MATCH (d:Disease)-[:NEEDS_CHECK]->(c:CheckItem)
            WHERE d.name = $disease
    ...
    "cured_prob": {
        ...
        "cypher": "MATCH (n:Disease) WHERE n.name = $disease RETURN coalesce(n.cure_rate, '治愈率信息暂无') AS RES LIMIT 1",
